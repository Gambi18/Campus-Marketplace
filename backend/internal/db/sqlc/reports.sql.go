...
RETURNING id, reporter_id, product_id, reason, status, created_at, updated_at, details
`
...
const getReportCountBySeller = `-- name: GetReportCountBySeller :many
SELECT
    p.seller_id,
...
type GetReportCountBySellerRow struct {
	SellerID    uuid.UUID `json:"seller_id"`
	ReportCount int32     `json:"report_count"`
}

func (q *Queries) GetReportCountBySeller(ctx context.Context) ([]GetReportCountBySellerRow, error) {
	rows, err := q.db.QueryContext(ctx, getReportCountBySeller)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetReportCountBySellerRow
	for rows.Next() {
		var i GetReportCountBySellerRow
		if err := rows.Scan(&i.SellerID, &i.ReportCount); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const getReportCountBySellerID = `-- name: GetReportCountBySellerID :one
...
func (q *Queries) GetReportCountBySellerID(ctx context.Context, sellerID uuid.UUID) (int32, error) {
	row := q.db.QueryRowContext(ctx, getReportCountBySellerID, sellerID)
